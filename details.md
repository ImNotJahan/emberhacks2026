...
Reference material that the [README](README.md) leaves out.
...
Settings live under `bigBrother.*`:
...
## Running it live: more detail

### Status bar

The item at the left of the status bar always shows the current state:
...
Set `bigBrother.autoStart` to false and run:
...
To add a session to the calibration set:
1. Record it live, as described in [Running it live](README.md#running-it-live-one-click).
